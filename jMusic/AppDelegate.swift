...
class AppDelegate: UIResponder, UIApplicationDelegate, GIDSignInDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        var googlePlistDict: NSDictionary?
        if let path = Bundle.main.path(forResource: "GoogleService-Info", ofType: "plist") {
            googlePlistDict = NSDictionary(contentsOfFile: path)
        }
        if let dict = googlePlistDict {
            // Use your dict here
            GIDSignIn.sharedInstance().clientID = dict["CLIENT_ID"] as? String
            print("cid:"+GIDSignIn.sharedInstance().clientID)
